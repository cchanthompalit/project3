...
proc set_pins {digits {name "hex_digit" }} {
	...
	for {set i 0} {$i < $digits} {incr i} {
		...
		foreach lamp {a b c d e f g} {
			...
			set_location_assignment -to "${name}\[${i}\].${lamp}" -location "PIN_${location}"
			incr j
		}
	}
}
	
set_location_assignment -to "${new_adc_clock}/clk_10MHz" -location "PIN_N5"
set_location_assignment -to "${clock_50}/clk_50MHz" -location "PIN_P11"
set_location_assignment -to "${reset}/reset" -location "PIN_B8"
